...
function addRowCondiciones(event) {
    event.preventDefault()
    let rowCount = document.getElementById(`condicionesTable`).rows.length

    const condicionesTableBody = document.getElementById(`condicionesTableBody`)
    const tr = document.createElement('tr')

    condicionesTableBody.appendChild(tr)
    tr.id = `newRowCondiciones${rowCount}`
    tr.innerHTML =
        `
            <td>
                ${rowCount}
            ...
            <select name="description_condition_additional[]">
                ...
                <option value="Incluye Costos de Limpieza, sub-limitados a USD 250,000 toda y cada pérdida, parte del agregado Anual y no en adición">Incluye Costos de Limpieza, sub-limitados a USD 250,000 toda y cada pérdida, parte del agregado Anual y no en adición</option>
            ...
            <td>
                <button id="${rowCount}" type="button" class="btn btn-danger btn-xs btn-delete-condicion">
                    <span class="glyphicon glyphicon-remove" aria-hidden="true"></span>
                </button>
            </td>
        `
...
$(document).on('click', '.btn-delete-condicion', function (e) {
    e.preventDefault()

    let id = $(this).attr('id')

    $(`#newRowCondiciones${id}`).remove()
})