...
@section('tab_title')
    ...
    <script>
        function updateClock() {
            ...
            const dateString =
                `Editar Broker | ${day}, ${date} de ${month} del ${year} ${hour}:${minute}:${second}`;
...
@endsection
@section('page_title')
    Administración | Broker de Reaseguros
@endsection
...
<div class="card">
    <div class="card-header">
        <ul class="nav nav-tabs">
            <li class="nav-item">
                <a class="nav-link" href="{{ route('brokers.index') }}"><i class="fa-solid fa-house"></i> Inicio</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="{{ route('brokers.create') }}"> <i class="fa-solid fa-plus"></i> Crear</a>
            </li>
        </ul>
    </div>
    ...
    <form method="POST" action="{{ route('brokers.update', ['id' => $brokers->id]) }}">
        @csrf
        @method('PUT')
        <input type="hidden" name="id" value="{{ $brokers->id }}">
        ...
        <div class="card-body" style="padding-block: 2rem">
                ...
                <div class="col-md-12">
                    <div class="col-md-4">
                        <div class="input-group">
                            ...
                            <input type="text" class="form-control" name="name" value="{{ $brokers->name }}">
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="input-group">
                            ...
                            <select id="select_country" class="js-example-basic-single form-select" name="country_id">
                                <option value="{{ $brokers->country_id }}" selected="selected">
                                    @if(is_numeric($brokers->country_id))
                                    {{ DB::table('countries')->select('name')->where('id', $brokers->country_id)->first()->name }}
                                    @else
                                    {{ $brokers->country_id }}
                                    @endif
                                </option>
                    ...
                    <div class="col-md-4">
                        <div class="input-group">
                            ...
                            <select id="select_position" class="js-example-basic-single form-select" name="position_id">
                                <option value="{{ $brokers->position_id }}" selected="selected">
                                    @if(is_numeric($brokers->position_id))
                                    {{ DB::table('positions')->select('name')->where('id', $brokers->position_id)->first()->name }}
                                    @else
                                    {{ $brokers->position_id }}
                                    @endif
                                </option>
                ...
                <div class="col-md-12">
                    <div class="col-md-4">
                        <div class="input-group">
                            ...
                            <input type="text" disabled class="form-control" value="BQ" reference>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="input-group">
                            ...
                            <select id="select_contact" class="js-example-basic-single inputForm" name="contact">
                                <option value="{{ $brokers->contact_id }}" selected="selected">
                                    {{ $brokers->contact_id }}</option>
                            </select>
                    ...
                    <div class="col-md-4">
                        <div class="input-group">
                            ...
                            <input type="email" class="form-control" name="email" value="{{ $brokers->email }}">
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="col-md-4">
                        <div class="input-group">
                            ...
                            <select id="select_region" class="js-example-basic-single form-select" name="region_id">
                                <option value="{{ $brokers->region_id }}">{{ $brokers->region_id }}</option>
                            </select>
            ...
            <div class="row">
                ...
                <div class="text-center my-4">
                    <a href="javascript:void(0)" data-id="{{ $brokers->id }}"
...
<script src="{{ asset('js/admin/database/broker.js') }}"></script>
